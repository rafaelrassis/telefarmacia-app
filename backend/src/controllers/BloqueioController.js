...
export const criarBloqueio = async (req, res) => {
  ...
  try {
    if (!forcar) {
      const agendadasComFarm = await prisma.filaAgendada.findMany({
        where: {
      ...
      const conflitos = agendadasComFarm.map((f) => ({
        id:          f.id,
        tipo:        'agendada',
        dataHora:    f.dataHora,
    ...
    if (forcar) {
      const agAfetadas = await prisma.filaAgendada.findMany({
        where: {
          farmaceuticoId: pharmacistId,
