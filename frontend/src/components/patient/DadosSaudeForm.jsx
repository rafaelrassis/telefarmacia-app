...
const DadosSaudeForm = ({ personId = null, onClose, onSaved }) => {
  ...
  if (loading) {
    return <p className="text-sm text-gray-400 text-center py-6">Carregando...</p>;
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-xs font-semibold text-gray-600 mb-1">
          Alergias conhecidas <span className="font-normal text-gray-400">(opcional)</span>
        </label>
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-wash focus:border-brand outline-none resize-none"
        />
      </div>
