...
const ScheduleManager = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Online toggle */}
      <div className={`rounded-2xl border p-5 transition-all duration-200 ${isOnline ? 'border-success/30 bg-success-wash' : 'border-line bg-surface'}`}>
      ...
      <div className="bg-canvas border border-line rounded-2xl overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <select
                    ...
                    className="border border-line rounded-lg px-2 py-1.5 text-sm text-ink bg-canvas focus:ring-2 focus:ring-brand outline-none disabled:cursor-not-allowed"
                  >
        ...
        <div className="px-5 py-4 border-t border-line bg-surface flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="shrink-0 bg-brand hover:bg-brand-deep disabled:opacity-50 text-brand-contrast font-bold text-sm px-5 py-2.5 rounded-xl transition shadow-sm"
          >
      ...
      {/* Message */}
      {msg && (
        <div className={`text-sm px-4 py-3 rounded-xl border ${msg.type === 'success' ? 'bg-success-wash border-success/30 text-success' : 'bg-error-wash border-error/30 text-error'}`}>
          {msg.text}
        </div>
      )}
                    ...
                    {daySlots.map((slot) => (
                      <div
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm ${
                          slot.isBooked
                            ? 'bg-brand-wash border-brand/30 text-brand-deep'
