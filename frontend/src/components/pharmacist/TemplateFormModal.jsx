...
const TemplateFormModal = ({ initial, onClose, onSaved }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-canvas rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-line rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-line rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand resize-y font-mono"
            />
          ...
          {err && (
            <p className="text-xs text-error bg-error-wash border border-error/20 rounded-xl px-4 py-3">{err}</p>
          )}
          ...
          <div className="flex gap-3 justify-end pt-1">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-5 py-2 text-sm font-bold bg-brand hover:bg-brand-deep text-white rounded-xl transition disabled:opacity-50"
            >
